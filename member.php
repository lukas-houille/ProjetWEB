...
if(isset($_SESSION["login"]) && $_SESSION["login"]->checkLogin()) {
    ...
    if ($type[0] == "Student") {
        ...
        $data = $db->executeQuery("SELECT description, Company.name as company, City.name as location FROM Internship_offer INNER JOIN Company ON Internship_offer.id_company=Company.id_company INNER JOIN City ON Internship_offer.id_city=City.id_city INNER JOIN wishes ON wishes.id_offer=Internship_offer.id_offer INNER JOIN Student ON Student.id_student=Wishes.id_student WHERE Student.id_student = :id;",["id" => $id[0]->id_student],return_option: PDO::FETCH_OBJ);
    } elseif($type[0] == "Admin"){
        ...
        $data = $db->executeQuery("SELECT description, Company.name as company, City.name as location FROM Internship_offer INNER JOIN Company ON Internship_offer.id_company=Company.id_company INNER JOIN City ON Internship_offer.id_city=City.id_city INNER JOIN admin_wishes ON admin_wishes.id_offer=Internship_offer.id_offer INNER JOIN Admin ON Admin.id_admin=admin_wishes.id_admin WHERE Admin.id_admin = :id;",["id" => $id[0]->id_admin],return_option: PDO::FETCH_OBJ);
    }// show all the cards in favorite with mustache template
    $cards = "";
